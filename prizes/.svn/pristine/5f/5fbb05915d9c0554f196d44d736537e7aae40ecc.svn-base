 ...
 *
 */
public class K33THZXDSAwardProcesser extends AwardProcesserImpl implements K3AwardProcesser{
	...
	public List<Award> prize(String openNum, String lottery_number)
		...
		if(lottery_number.matches("(([0-9][,]){2}[0-9][#]){0,4}([0-9][,]){2}[0-9]")){									
			...
			for (String string : lotteryNums) {
				Award award = AwardUtils.prizewinning_K3(string.split(","), open, AwardUtils.K3_三同号自选单式);		
				if (award != null)
					list.add(award);
			}	
			return list;
