 ...
 *
 */
public class K32THZXFSAwardProcesser extends AwardProcesserImpl implements K3AwardProcesser{
	...
	public List<Award> prize(String openNum, String lottery_number)
		...
		if(lottery_number.matches("(([0-9][,]){1}[0-9][#]){0,4}([0-9][,]){1}[0-9]")){									
			...
			for (String string : lotteryNums) {
				...
				Award award = AwardUtils.prizewinning_K3(arr1, open, AwardUtils.K3_二同号自选复式);		
				if (award != null)
					list.add(award);
			}	
			return list;
